...
//          }
    ...
    menu.addSeparator();
    item = new JMenuItem(_("Galileo Firmware Update"));
    item.addActionListener(new ActionListener() {
       public void actionPerformed(ActionEvent e) {
               handleUpdate();
       }
...
//    }
  ...
  private boolean cmd_status;

  private boolean testFlag = false;
  private String hlabel = "Arduino Firmware Update";
  private String wlabel ="<font color='red'>Warning: do not interrupt by disconnecting power or USB, or by pressing any microswitches!</font>";

  private boolean showProgressDialog(final String[] cmd, String label) {

        final JDialog dlg = new JDialog(this, hlabel, true);
        final JProgressBar dpb = new JProgressBar(JProgressBar.HORIZONTAL);
        dpb.setIndeterminate(true);
        dlg.add(BorderLayout.CENTER, dpb);
        dlg.add(BorderLayout.NORTH, new JLabel("<html><p>"+label+"</p><br><p>"+wlabel+"</p></html>"));
        ...
        Thread t = new Thread(new Runnable() {
            public void run() {
                    cmd_status = executeCommand(cmd, false);
  ...
  private boolean showProgress(final String[] cmd, String label){
      result  = new AtomicBoolean();
      dialog = new JDialog(this, hlabel, true);
      JProgressBar dpb = new JProgressBar(JProgressBar.HORIZONTAL);
      dpb.setIndeterminate(true);
      dialog.add(BorderLayout.CENTER, dpb);
      dialog.add(BorderLayout.NORTH, new JLabel("<html><p>"+label+"</p><br><p>"+wlabel+"</p></html>"));
        ...
        Thread t = new Thread(new Runnable() {
            public void run() {
                ...
                for(;;) {
                    ...
                    if(start_polling <= (System.currentTimeMillis() - startTime )) {
                        result.set(executeCommand(cmd, false));
                        if (Preferences.getBoolean("upload.verbose")) {
                           System.out.println("poll ...  "+ ++i);
                        }
                    }

                    if(result.get() || duration <= (System.currentTimeMillis() - startTime )) {
                        if (Preferences.getBoolean("upload.verbose")) {
                            if(result.get()) {
                                System.out.println("Poll success");
                            }else {
                                System.out.println("Timeout Polling target");
                            }
                        }
                        break;
                    }
  ...
  private boolean executeCommand(String[] cmd, boolean skipOutputCheck) {
      int ret = -1;
    try{
          ...
          if(ret == 0){
              ...
              if(skipOutputCheck || output.toString().contains("Transfer complete")) {
                        return true;
              }
          }
    } catch(Exception er) {
  ...
  private String fileFinder(String dirName) {
  ...
  private String getImageVersion(String filePath) throws IOException {
      ...
      int index_FL_IMG_VAL = FL_IMG_VAL.length() + fileData.indexOf(FL_IMG_VAL, index_FL_IMG_VER);
      int index_FL_IMG_EOL = fileData.indexOf(FL_IMG_EOL,index_FL_IMG_VAL);
      String version = fileData.substring(index_FL_IMG_VAL,index_FL_IMG_EOL);
  ...
  private String readFileAsString(String filePath) throws IOException {
   ...
   private String getMD5Checksum(String filePath){
    ...
    static private String executeRemoteCommandStr = "execute remote command";
    static private String transferCompleteStr = "Transfer complete";

    private String doVersionQuery(String[] cmd) {
        String version = null;

        if (this.executeCommand(cmd, false)) {
            String response = output.toString();
            int responseStart = response.indexOf(executeRemoteCommandStr);
            int responseEnd = response.lastIndexOf(transferCompleteStr);

            if (responseStart >= 0 || responseEnd >= 0) {
                responseStart += executeRemoteCommandStr.length();
                version = response.substring(responseStart, responseEnd).trim();
            }
        }

        if (version == null) {
            JOptionPane.showMessageDialog(null, "Target firmware version query failed.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    ...
    private int handleUpdate() {
        ...
        String comPort = Preferences.get("serial.port");

        try {
                String sys_file_name;
                String sys_file_version;
                String sys_file_md5;
                ...
                String exec_path;
                if(Base.isWindows()) {
                    exec_path = current_dir.replace("\\","/")+"/hardware/tools/x86/bin";
                }else if (Base.isMacOS()) {
                    exec_path = Base.getHardwarePath() + "/tools/x86/bin";
                }else {
                    exec_path = current_dir.replace("\\","/")+"/hardware/tools";
                }

                String bash_path;
                if(Base.isWindows()) {
                    bash_path = exec_path+"/bash.exe";
                } else {
                    bash_path = "bash";
                }

                windowReturn = JOptionPane.showConfirmDialog(null,
                    "Intel Galileo firmware update requires using the external power supply.",
                    "Arduino Firmware Update", JOptionPane.OK_CANCEL_OPTION);

                if(JOptionPane.CANCEL_OPTION == windowReturn || JOptionPane.CLOSED_OPTION == windowReturn) {
                    //System.out.println("Exit without update");
                    return 0;
                }

                if((sys_file_name = fileFinder(exec_path))== null) {
                    JOptionPane.showMessageDialog(null, "New firmware image not found.\nPlease visit the Intel Galileo website for updates.", "Error", JOptionPane.ERROR_MESSAGE);
                    return 0;
                } else {
                    //System.out.println("file name = "+sys_file_name+" length = "+sys_file_name.length());
                }

                if((sys_file_version = getImageVersion(exec_path+"/"+sys_file_name)) == null) {
                    JOptionPane.showMessageDialog(null, "New firmware image version not found.", "Error", JOptionPane.ERROR_MESSAGE);
                    return 0;
                }

                GalileoVersionId ready_version_id = GalileoVersionId.ofTargetString(sys_file_version);

                if((sys_file_md5 = getMD5Checksum(exec_path+"/"+sys_file_name))==null)  {
                    JOptionPane.showMessageDialog(null, "Unable to calculate md5 for new image", "Error", JOptionPane.ERROR_MESSAGE);
                    return 0;
                } else {
                    //System.out.println("file md5 = "+sys_file_md5+" length = "+sys_file_md5.length());
                }
            ...
            /****************************************Remote commands****************************************/
                ...
                String[] cmd_version = {bash_path, "--noprofile", exec_path+"/upgrade.sh",
                                        exec_path, sys_file_name, comPort,
                                        "cat", "/sys/firmware/board_data/flash_version"};

                String[] cmd_download = {bash_path, "--verbose", "--noprofile", exec_path+"/upgrade.sh",
                                         exec_path, sys_file_name, comPort};

                String[] cmd_md5sum = {bash_path, "--noprofile", exec_path+"/upgrade.sh",
                                        exec_path, sys_file_name, comPort,
                                        "md5sum", "/"+sys_file_name};

                String[] cmd_mkdir1 = {bash_path, "--noprofile", exec_path+"/upgrade.sh",
                                       exec_path, sys_file_name, comPort,
                                       "mkdir", "-p", "/tmp/spi_upgrade"};

                String[] cmd_mkdir2 = {bash_path, "--noprofile", exec_path+"/upgrade.sh",
                                       exec_path, sys_file_name, comPort,
                                       "mkdir", "-p", "/lib/firmware/tmp/spi_upgrade"};

                String[] cmd_mv = {bash_path, "--noprofile", exec_path+"/upgrade.sh",
                                   exec_path, sys_file_name, comPort,
                                   "mv", "/"+sys_file_name, "/tmp/spi_upgrade/galileo_firmware.bin"};

                String[] cmd_cp = {bash_path, "--noprofile", exec_path+"/upgrade.sh",
                                    exec_path, sys_file_name, comPort,
                                    "cp", "/tmp/spi_upgrade/galileo_firmware.bin",
                                    "/lib/firmware/tmp/spi_upgrade/galileo_firmware.bin"};

                String[] cmd_upgrade = {bash_path, "--noprofile", exec_path+"/upgrade.sh",
                                        exec_path, sys_file_name, comPort,
                                        "./opt/cln/galileo/start_spi_upgrade.sh"};

                String[] cmd_reboot = {bash_path, "--noprofile", exec_path+"/upgrade.sh",
                                       exec_path, sys_file_name, comPort,
                                        "reboot"};
                ...
                String ver = this.doVersionQuery(cmd_version);
                if (ver == null) {
                    return 0;
                }

                GalileoVersionId target_version_id = GalileoVersionId.ofTargetString(ver);
                String windowDescript = "Target firmware is version '" + target_version_id.toPresentationString() + "' now.\n\nDo you wish to ";
                Boolean isEquivalent = false;

                try {
                    int d = ready_version_id.compareTo(target_version_id);
                    if (d < 0) {
                        windowDescript += "update with older";
                    }
                    else if (d > 0) {
                        windowDescript += "update with newer";
                    }
                    else {
                        windowDescript += "rewrite with equivalent";
                        isEquivalent = true;
                    }
                }
                catch (IllegalArgumentException _) {
                    windowDescript += "overwrite with";
                }

                if (!isEquivalent) {
                    windowDescript += " version '" + ready_version_id.toPresentationString() + "'";
                }

                windowDescript += " firmware?";

                windowReturn = JOptionPane.showConfirmDialog(null, windowDescript, "Arduino Firmware Update", JOptionPane.YES_NO_OPTION);
                if(JOptionPane.NO_OPTION == windowReturn || JOptionPane.CLOSED_OPTION == windowReturn) {
                    //System.out.println("Exit without update");
                    return 0;
                }

                //System.out.println("DOWNLOADING NEW FIRMWARE ONTO TARGET.... \n!!!DO NOT TOUCH THE BOARD!!!");
                if(!showProgressDialog(cmd_download,"Downloading new firmware onto target.")) {
                    JOptionPane.showMessageDialog(null, "Error occured while downloading new firmware onto target", "Arduino Firmware Update", JOptionPane.ERROR_MESSAGE);
                    return 0;
                }

                if(!executeCommand(cmd_md5sum, false)) {
                    JOptionPane.showMessageDialog(null, "Error occured querying md5sum from target", "Arduino Firmware Update", JOptionPane.ERROR_MESSAGE);
                    return 0;
                }
                //System.out.println("VERIFYING CHECKSUM ... \n!!!DO NOT TOUCH THE BOARD!!!");
                //System.out.println("sys_file_md5="+sys_file_md5);
                int sIndex = output.lastIndexOf("/"+sys_file_name);
                //System.out.println("md5 = "+output.substring(sIndex-34, sIndex-2));
                if(!sys_file_md5.equalsIgnoreCase(output.substring(sIndex-34, sIndex-2))) {
                    JOptionPane.showMessageDialog(null, "Checksum mismatch error", "Arduino Firmware Update", JOptionPane.ERROR_MESSAGE);
                    return 0;
                }


                if(!executeCommand(cmd_mkdir1, false)){
                    JOptionPane.showMessageDialog(null, "Firmware update error: mkdir [1] ", "Error", JOptionPane.ERROR_MESSAGE);
                    return 0;
                }

                if(!executeCommand(cmd_mkdir2, false)){
                    JOptionPane.showMessageDialog(null, "Firmware update error: mkdir [2] ", "Error", JOptionPane.ERROR_MESSAGE);
                    return 0;
                }

                if(!executeCommand(cmd_mv, false)){
                    JOptionPane.showMessageDialog(null, "Firmware update error: mv", "Error", JOptionPane.ERROR_MESSAGE);
                    return 0;
                }

                if(!executeCommand(cmd_cp, false)){
                    JOptionPane.showMessageDialog(null, "Firmware update error: cp", "Error", JOptionPane.ERROR_MESSAGE);
                    return 0;
                }

                //System.out.println("TRIGGERING UPGRADE ... \n!!!DO NOT TOUCH THE BOARD!!!");
                if(!showProgressDialog(cmd_upgrade, "Preparing target for update.")) {
                    JOptionPane.showMessageDialog(null, "Firmware update trigger failure", "Error", JOptionPane.ERROR_MESSAGE);
                    return 0;
                }

                if (!output.toString().contains("Upgrade trigger success")) {
                    JOptionPane.showMessageDialog(null, "Firmware update trigger unsuccessful", "Error", JOptionPane.ERROR_MESSAGE);
                    return 0;
                }

                //System.out.println("Rebooting the target now.....\n Update process will take about 4-5 minutes. \n !!!Please Wait!!!!");
                if(!showProgressDialog(cmd_reboot, "Rebooting target for update ")) {
                    //JOptionPane.showMessageDialog(null, "Firmware update error : reboot command" , "Error", JOptionPane.ERROR_MESSAGE);
                    //return 0;
                }

                if(!showProgress(cmd_version, "Updating firmware...This may take up to 5 minutes.")) {
                    JOptionPane.showMessageDialog(null, "Target firmware update failed", "Error", JOptionPane.ERROR_MESSAGE);
                    return 0;
                }

                ver = this.doVersionQuery(cmd_version);
                if (ver == null) {
                    return 0;
                }

                GalileoVersionId installed_version_id = GalileoVersionId.ofTargetString(ver);
                String msgText = "Target firmware not updated.";
                int msgType = JOptionPane.ERROR_MESSAGE;

                try {
                    int d = installed_version_id.compareTo(ready_version_id);
                    if (d == 0) {
                        msgText = "Target firmware successfully updated.";
                ...
                JOptionPane.showMessageDialog(null, msgText, "Firmware Update", msgType);
        }
        catch(Exception er) {
            er.printStackTrace();
        }
